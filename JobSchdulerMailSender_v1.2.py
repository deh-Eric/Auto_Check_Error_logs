...
'''
    UPDATE TIME: 2021-08-12 12:00:00    
    mailSender()：邮件发送模块  
    folderMonitor()：文件修改时间监测模块
    errChecker(): 文件内容检查模块
    main():主模块
'''
...
"""
def getOriginTimeSet():
    '''
        获得所有修改日期在2021以后修改过的文件的时间戳，存入集合
    '''
    start_detection_time = 1609430400 # input("Please input start detect timepoint")
    timeset = set() # 集合 用于存储存储程序开始运行时各文件的状态
    path = u"D:\ProgramData\sos-berlin.com\jobscheduler\shaappp01128_4444\logs"
    for root, dir, files in os.walk(path):
        for file in files:
            full_path = os.path.join(root, file)
            mtime = os.stat(full_path).st_mtime
            if mtime > int(start_detection_time):
                timeset.add(mtime)
    return timeset
"""
...
def folderMonitor(logs_path,now_time):
    ...
    for root, dir, files in os.walk(path):
        for file in files:
            ...
            mtime = os.stat(full_path).st_mtime
            # print(file,mtime)
            if mtime > now_time and (file.startswith('task.Mercedes_Start') or file.startswith('task.PRC_Start') or file.startswith('task.Chanel_Start')):
                filelist.add(file)
    return filelist
...
if __name__ == '__main__':
    main()
    # pprint(load_config_param()['receives'])